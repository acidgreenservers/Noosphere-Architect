    ...
    <Modal isOpen={isOpen} onClose={onClose} title={title} maxWidthClass="max-w-6xl">
      <div className="flex flex-col h-full animate-fade-in">
        ...
        <div className="flex justify-end items-center gap-3 mt-8 pt-4 border-t border-slate-100 dark:border-slate-800/80">
           ...
           <button
              onClick={handleCopy}
              className={`flex items-center px-4 py-2 border rounded-xl text-xs font-semibold transition cursor-pointer ${copyStatus ? 'bg-green-500/10 text-green-600 dark:text-green-400 border-green-500/20' : 'border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-900/60'}`}
              title="Copy to Clipboard"
            >
              <span className="material-icons text-sm mr-1.5">{copyStatus ? 'check' : 'content_copy'}</span>
              {copyStatus ? 'Copied' : 'Copy'}
            </button>
            <button
              onClick={onExport}
              className="flex items-center px-4 py-2 border border-slate-200 dark:border-slate-800 rounded-xl text-xs font-semibold text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-900/60 transition cursor-pointer"
