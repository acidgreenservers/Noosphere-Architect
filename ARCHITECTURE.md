...
+-----------+-----------+     +-----------+-----------+     +-----------+-----------+
          ...
          +-----------------------------------------------------------+
          |                      Persistence Layer                    |
          |           (IndexedDB v19 + Metadata Unification)          |
          +-----------------------------+-----------------------------+
                                        |
              +-------------------------+-------------------------+
              |                         |                         |
      ...
      [Compressed Signals]      [Seed Architects]         [Draft & Pref Stores]
```
...
* **IndexedDB Migration Registry:** `dbService.ts` implements a versioned
  migration system (currently v19) to ensure data integrity as the schema
  evolves. Specifically, the schema transitions handle legacy metadata unification (v11), roadmap tracking (v13), employer handbook agent jobs (v15), compressed signal / seed architecture (v16), seed drafts (v17), user preferences persistence (v18), and explicit `sourceTool` record stamping (v19).
